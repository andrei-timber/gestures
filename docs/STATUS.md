...
- **Since M1 (post-milestone polish, 2026-07-06):** owner-feedback batch shipped & redeployed
  (Version `3a4554bb`) — in-session **Refresh image** (`f`) + **Continue the pose** (no-timer free-draw
  from the recap), setup **FYI full-sequence** line, number-field **select-on-focus**, and **wider source
  cards + filled-Drive-link** cue. Five separate commits; spec §6 + decisions logged. Not an M2 step.
...
- **Verify:** live — `curl https://andreitim.com/apps/gestures/` → **200** (Version `3a4554bb`,
  2026-07-06). Gate green — **182 tests, typecheck, lint**. M1 iPad run confirmed by owner (2026-07-04);
  the polish batch is desktop browser-verified — owner to spot-check on iPad.
